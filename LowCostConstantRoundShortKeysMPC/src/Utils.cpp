...
void Utils::roundFunction(vector<byte> & data, int sendParty) {
    if (id == sendParty) {
        //send the data to the other parties
        for (int i = 0; i < parties.size(); i++) {
            parties[i]->getChannel()->write((byte *) data.data(), data.size());
        }
    } else {
        //Receive the data from the send party
        if (sendParty > id) { // the id of the send party is bigger than my id
            parties[sendParty - 1]->getChannel()->read(data.data(), data.size());
        } else {//the id of the send party is smaller than my id
            parties[sendParty]->getChannel()->read(data.data(), data.size());
        }

    }
}

void Utils::roundFunction(vector<vector<byte>> & data, int sendParty) {
    if (id == sendParty) {
        //send the data to the other parties
        for (int i = 0; i < parties.size(); i++) {
            parties[i]->getChannel()->write((byte *) data[i].data(), data[i].size());
        }
    } else {
        //Receive the data from the send party
        if (sendParty > id) { // the id of the send party is bigger than my id
            parties[sendParty - 1]->getChannel()->read(data[0].data(), data[0].size());
        } else {//the id of the send party is smaller than my id
            parties[sendParty]->getChannel()->read(data[0].data(), data[0].size());
        }
